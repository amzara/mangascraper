...
func (s *DownloadService) DeleteManga(mangaID int32) error {
	ctx := context.Background()

	// Get manga details first so we know which directory to delete
	manga, err := s.Queries.GetMangaByID(ctx, mangaID)
	if err != nil {
		return fmt.Errorf("failed to get manga: %w", err)
	}

	// Delete from DB (cascades to chapters and pages)
	if err := s.Queries.DeleteManga(ctx, mangaID); err != nil {
		return fmt.Errorf("failed to delete manga from DB: %w", err)
	}

	// Delete local files
	mangaDir := filepath.Join("downloads", manga.Slug)
	if err := os.RemoveAll(mangaDir); err != nil {
		return fmt.Errorf("failed to delete manga files: %w", err)
	}
