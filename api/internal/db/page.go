...
func SavePage(chapterID int, imageURL, fileName, filePath string) (int, error) {
	var pageID int
	err := DB.QueryRow(`
		...
		RETURNING page_id
	`, chapterID, imageURL, fileName, filePath).Scan(&pageID)
...
func GetPagesByChapter(chapterID int) ([]Page, error) {
	...
	for rows.Next() {
		var p Page
		var errorMsg string
		err := rows.Scan(&p.PageID, &p.ChapterID, &p.ImageURL, &p.FileName, &p.FilePath,
