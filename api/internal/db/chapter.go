...
func SaveChapter(mangaID int, chapterSlug string, chapterNum int) (int, error) {
	var chapterID int
	err := DB.QueryRow(`
	...
	`, mangaID, chapterSlug, chapterNum, ChapterStatusPending).Scan(&chapterID)

	if err != nil {
		// Check if it's a "no rows" error (chapter already exists with no RETURNING)
		...
		fmt.Printf("    Chapter saved: %s (num: %d, ID: %d)\n", chapterSlug, chapterNum, chapterID)
	}
...
func GetChapterByID(chapterID int) (*Chapter, error) {
	...
	err := DB.QueryRow(`
		...
		WHERE chapter_id = $1
	`, chapterID).Scan(
		&c.ChapterID, &c.MangaID, &c.ChapterSlug, &c.ChapterNum, &statusStr,
...
func GetChaptersByManga(mangaID int) ([]Chapter, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&c.ChapterID, &c.MangaID, &c.ChapterSlug, &c.ChapterNum, &statusStr,
			&errorMsg, &c.CreatedAt, &updatedAt)
		if err != nil {
			continue
		}
...
func GetPendingChapters() ([]Chapter, error) {
	...
	for rows.Next() {
		var c Chapter
		err := rows.Scan(&c.ChapterID, &c.MangaID, &c.ChapterSlug, &c.ChapterNum, &c.Status, &c.CreatedAt)
		if err != nil {
			continue
		}
		chapters = append(chapters, c)
